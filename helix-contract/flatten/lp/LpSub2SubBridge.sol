...
contract LpBridgeBacking is LpBridgeHelper {
    ...
    function _withdrawLiquidity(
        ...
        uint256 lpFee = amount + fee - helixFee;
        if (withdrawNative && tokenIndex == wTokenIndex) {
            IWToken(tokenInfo.localToken).withdraw(lpFee);
            payable(receiver).transfer(lpFee);
        } else {
            _safeTransfer(tokenInfo.localToken, receiver, lpFee);
        }
...
// File contracts/mapping-token/v2/lp/LpSub2SubBridge.sol
// License-Identifier: MIT
