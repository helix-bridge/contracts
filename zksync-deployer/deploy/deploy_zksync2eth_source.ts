...
export default async function (hre: HardhatRuntimeEnvironment) {
  ...
  console.log(`Running deploy script for the zksync2eth source contract`);
  ...
  const artifact = await deployer.loadArtifact("ZkSync2EthSource");
  ...
  const logicVerificationId = await hre.run("verify:verify", {
      ...
      contract: "contracts/ln/ZkSync2EthSource.sol:ZkSync2EthSource",
